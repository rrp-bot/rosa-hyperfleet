...
- **Problem Statement**: Quay.io has experienced repeated availability incidents that have caused
  image pull failures across Management Clusters (MCs) and Regional Clusters (RCs). Because the
  platform's operational images are sourced from Quay at runtime, any Quay degradation directly
  impacts cluster stability, component rollouts, and the ability to provision or upgrade Hosted
  Clusters. This is an unacceptable operational dependency for a managed service.

  There is a second, distinct motivation: zero-egress Hosted Control Plane environments cannot
...
- **Constraints**:
  ...
  - Konflux is the build system for platform images; the mirroring pipeline must integrate with
    Konflux build outputs.
...
1. **Continue using Quay.io with retry/fallback logic**: Add pull-retry configuration and circuit
   breakers around Quay image pulls. This does not address the root cause (Quay availability) and
   provides no path to zero-egress support. Rejected.

2. **Mirror images to a self-managed registry (e.g., a registry running on the RC)**: Gives full
...
3. **Use ECR for platform images only, retain Quay for HCP dataplane images**: Partially improves
   stability but leaves the HCP dataplane dependent on Quay and does not support zero-egress
   control planes. Rejected.

4. **Use ECR for both image sets, separate authentication paths**: Viable but increases operational
   complexity. A unified auth path (see chosen approach) is preferred. Chosen only if unified auth
   proves infeasible after investigation.

5. **Use ECR for both image sets with unified auth, reusing OCP mirroring infra for platform
   images**: Minimises new infrastructure, reuses proven tooling, and provides a single auth path
   for all ECR image pulls. **Chosen.**
...
2. **HCP dataplane pods inside Hosted Clusters**: Pods running inside Hosted Clusters need to pull
   images from ECR. This is more complex because these pods run with the Hosted Cluster's node
   identity, not the management cluster's identity. The mechanism by which ECR credentials are made
   available to HCP dataplane pods is not yet fully defined and requires investigation. Potential
   approaches include:
   - Propagating ECR credential secrets into the Hosted Cluster's control plane namespace and
     mounting them as pull secrets.
   - Using a credential rotation controller that injects short-lived ECR tokens into the relevant
     namespaces on the management cluster.
   - Leveraging HyperShift's existing pull secret handling to include an ECR token.

   The investigation should determine which approach is compatible with the HyperShift operator's
   lifecycle management and the platform's IAM model (Pod Identity / IRSA). The goal is that a
...
│                        Mirroring Pipeline                           │
│                                                                     │
│  OCP Release Process          Konflux                               │
│  ┌──────────────┐             ┌──────────────┐                      │
│  │ Quay.io      │             │ Platform      │                      │
│  │ OCP images   │             │ image builds  │                      │
│  └──────┬───────┘             └──────┬────────┘                     │
│         │                           │                               │
│         ▼                           ▼                               │
│  ┌─────────────────────────────────────────┐                        │
│  │     OCP Mirroring Pipeline (extended)   │                        │
│  │     - OCP image set (existing)          │                        │
│  │     - Platform image set (new)          │                        │
│  └──────────────────────┬──────────────────┘                        │
│                         │                                           │
│                         ▼  (per region)                             │
│               ┌──────────────────┐                                  │
...
- Eliminates runtime dependency on Quay.io for both HCP dataplane images and platform operational
  images, removing Quay availability as a failure mode for the platform.
- Enables zero-egress Hosted Control Plane support by sourcing all HCP dataplane images from
  regional ECR without activating the broader `--zero-egress` mode.
...
- Authentication for HCP dataplane pods is not yet resolved. Until the auth mechanism is
  confirmed, there is a risk that ECR image pulls for control plane components require a
  non-trivial credential injection approach that interacts poorly with HyperShift's lifecycle
  management.
...
- ECR token lifetimes (12 hours) require active rotation; any failure in credential refresh will
  cause pull failures.
...
- ECR image scanning should be enabled to provide vulnerability visibility on platform images as
  they are mirrored.
